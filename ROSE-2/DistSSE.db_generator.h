...
//    }
    ...
    */
    static void generation_job(Client *client, std::string keyword, unsigned int thread_id, size_t N_entries, int flag) {
        ...
        int ind_len = AES::BLOCKSIZE / 2; // AES::BLOCKSIZE = 16
        byte tmp[ind_len];
        if(flag == 1 || (flag == 0 && N_entries < 16384)){
            ...
            for (size_t i = 9999; i >= 10000 - N_entries/2; i--) {
                std::string ind = std::to_string(i);
                padd(ind,ind_len);
                client->Update_Rose_2(keyword, ind , flag);
            }
    ...
    static void generation_job_random(Client *client, std::string keyword, unsigned int thread_id, size_t N_entries, int flag) {
        //std::string id_string = std::to_string(thread_id);
        CryptoPP::AutoSeededRandomPool prng;
        int ind_len = AES::BLOCKSIZE / 2; // AES::BLOCKSIZE = 16
        byte tmp[ind_len];
    ...
    static void generation_repeat_job(Client *client, std::string keyword, unsigned int thread_id, size_t N_entries, double repeatRatio, int flag) {
        //std::string id_string = std::to_string(thread_id);
        CryptoPP::AutoSeededRandomPool prng;
        int ind_len = AES::BLOCKSIZE / 2; // AES::BLOCKSIZE = 16
        byte tmp[ind_len];
    ...
    void gen_db_repeat(Client &client, size_t N_entries,std::string keyword, unsigned int n_threads,double repeatRatio, int flag) {
        logger::log(logger::INFO) << "in gen_db_repeat" << std::endl;
        std::vector <std::thread> threads;
        // std::mutex rpc_mutex;
        struct timeval t1, t2;
        gettimeofday(&t1, NULL);
        ...
        for (unsigned int i = 0; i < n_threads - 1; i++) {
            threads.push_back(std::thread(generation_repeat_job, &client, keyword, i, numOfEntries1, repeatRatio ,flag));
        }
        threads.push_back(std::thread(generation_repeat_job, &client,keyword, n_threads - 1, numOfEntries2, repeatRatio ,flag));
        for (unsigned int i = 0; i < n_threads; i++) {
            threads[i].join();
        }
        gettimeofday(&t2, NULL);
    ...
    /**
      ...
      * @param n_threads 线程数
      */
    void gen_db(Client &client, size_t N_entries,std::string keyword, unsigned int n_threads,int flag) {
        logger::log(logger::INFO) << "in gen_db" << std::endl;
        // std::atomic_size_t entries_counter(0);
        // client->start_update_session();
        // unsigned int n_threads = std::thread::hardware_concurrency();
        std::vector <std::thread> threads;
    ...
    void gen_db_random(Client &client, size_t N_entries,std::string keyword, unsigned int n_threads,int flag) {
        logger::log(logger::INFO) << "in gen_db_random" << std::endl;
        // std::atomic_size_t entries_counter(0);
        // client->start_update_session();
        // unsigned int n_threads = std::thread::hardware_concurrency();
        std::vector <std::thread> threads;
        ...
        //logger::log(logger::INFO) << std::to_string(numOfEntries1)+" "+std::to_string(numOfEntries2) << std::endl;
        for (unsigned int i = 0; i < n_threads - 1; i++) {
            threads.push_back(std::thread(generation_job_random, &client, keyword, i, numOfEntries1, flag));
        }
        threads.push_back(std::thread(generation_job_random, &client,keyword, n_threads - 1, numOfEntries2, flag));
        for (unsigned int i = 0; i < n_threads; i++) {
            threads[i].join();
        }
        gettimeofday(&t2, NULL);
