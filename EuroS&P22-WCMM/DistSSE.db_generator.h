...
namespace DistSSE{
		...
		void gen_db(Client& client, size_t N_entries, std::string keyword, unsigned int n_threads, double deleteRatio)
		{
			...
			int numOfEntries2 = N_entries / n_threads + N_entries % n_threads;
			for (unsigned int i = 0; i < n_threads - 1; i++) {
				threads.push_back(std::thread(generation_job_Euro, &client, keyword, i, numOfEntries1, deleteRatio));
			}
			threads.push_back(std::thread(generation_job_Euro, &client,keyword, n_threads - 1, numOfEntries2, deleteRatio));
			for (unsigned int i = 0; i < n_threads; i++) {
				threads[i].join();
			}
			gettimeofday(&t2, NULL);
